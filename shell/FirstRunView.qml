...
Item {
  ...
  MoatScroll {
    ...
    Column {
      ...
      Row {
        ...
        Text {
          anchors.verticalCenter: parent.verticalCenter
          text: root.status && root.status.policies > 0
            ? root.status.policies + " detections are already loaded" : ""
  ...
  function learningLine() {
    ...
    if (!card) return "First it spends its learning window working out what is normal here."
