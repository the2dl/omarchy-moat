...
Item {
    ...
    readonly property var programs: root.service ? Model.trustedPrograms(root.tuples, root.incidents) : []
    ...
    readonly property var enforceable: root.service && root.service.status ? root.service.enforceableRules() : []
    readonly property var contained: root.service && root.service.status ? root.service.containments() : []
    readonly property var exclusions: root.service && root.service.status ? root.service.exclusions() : []
    readonly property int armedCount: {
        var n = 0;
        for (var i = 0; i < root.enforceable.length; i++) if (root.enforceable[i].armed) {
            n++;
        }
        return n;
    }
    ...
    MoatScroll {
        ...
        Column {
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        Column {
                            ...
                            Text {
                                width: parent.width
                                text: Model.basename(exRow.modelData.exe) + " is not watched by " + exRow.modelData.rule
                            ...
                            Text {
                                ...
                                // prompted it.
                                text: exRow.modelData.exe + " — for every path that rule watches, not just the one you allowed."
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        Column {
                            ...
                            Text {
                                width: parent.width
                                text: Model.basename(cRow.modelData.exe) + " may not reach " + cRow.modelData.dests.join(", ")
                                color: root.t ? root.t.body : "white"
                                font.family: root.t ? root.t.family : "monospace"
                                font.pixelSize: root.t ? root.t.fBody : 13
                                elide: Text.ElideRight
            ...
            Column {
                ...
                PanelSectionHeader {
                    text: "WHAT MOAT IS ALLOWED TO STOP  \u00b7  " + root.armedCount + " of " + root.enforceable.length
                }

                Text {
                    width: parent.width
                    text: "Everything else is watched and reported only. Arm one at a time, " + "and only after you have seen how often it fires here."
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        Text {
                            ...
                            color: armRow.modelData.armed ? (root.t ? root.t.accent : "orange") : (root.t ? root.t.faint : "grey")
                            font.family: root.t ? root.t.family : "monospace"
                            font.pixelSize: root.t ? root.t.fSecondary : 12
                            textFormat: Text.PlainText
                        }
                        ...
                        ToggleSwitch {
                            ...
                            onToggled: {
                                ...
                                root.service.setRuleMode(armRow.modelData.rule, armRow.modelData.armed ? "monitor" : "enforce");
                            }
                        }
            ...
            Column {
                ...
                Repeater {
                    ...
                    delegate: Rectangle {
                        ...
                        Column {
                            // A proposal Moat is NOT confident about.
                            ...
                            Text {
                                width: parent.width
                                text: modelData.count + (modelData.count === 1 ? " time" : " times") + " on " + modelData.days + (modelData.days === 1 ? " day" : " days") + "  ·  " + Model.ruleScopeWords(modelData.rule, modelData.dir)
                                color: root.t ? root.t.dimmer : "grey"
                                font.family: root.t ? root.t.family : "monospace"
                                font.pixelSize: root.t ? root.t.fSecondary : 12
                                wrapMode: Text.WordWrap
                            ...
                            // The two kinds are one keystroke apart and produce the same
                            // permanent allowlist entry, so the difference has to be on the
                            // card and not only in `moatctl`. Drawn in the warning tone
                            // rather than the quiet one: this is the row where "Stop asking"
                            // is a decision rather than a confirmation.
                            Text {
                                width: parent.width
                    ...
                    delegate: Rectangle {
                        ...
                        Item {
                            ...
                            Text {
                                ...
                                text: modelData.silenced < 0 ? "not counted" : "silenced " + modelData.silenced + " since"
                                color: root.t ? root.t.faint : "grey"
                                font.family: root.t ? root.t.family : "monospace"
                                font.pixelSize: root.t ? root.t.fMeta : 11
                                textFormat: Text.PlainText
                    ...
                    delegate: Item {
                        ...
                        Item {
                            ...
                            Text {
                                ...
                                color: modelData.trusted ? (root.t ? root.t.dimmer : "grey") : (root.t ? root.t.accent : "orange")
                                font.family: root.t ? root.t.family : "monospace"
                                font.pixelSize: root.t ? root.t.fSecondary : 12
                                elide: Text.ElideRight
                                wrapMode: Text.WordWrap
                    ...
                    delegate: Item {
                        ...
                        Item {
                            ...
                            Column {
                                ...
                                Text {
                                    ...
                                    text: {
                                        ...
                                        if (cut > 0)
                                            bits.push("taken from " + Model.shortenHome(dir.slice(0, cut)));

                                        var age = root.service ? root.service.relativeTime(modelData.when) : "";
                                        if (age)
                                            bits.push(age);

                                        if (!modelData.present)
                                            bits.push("no longer where Moat put it");

                                        return bits.join("  ·  ");
                                    }
                                    color: modelData.present ? (root.t ? root.t.faint : "grey") : (root.t ? root.t.alarm : "red")
                                    font.family: root.t ? root.t.family : "monospace"
                                    font.pixelSize: root.t ? root.t.fMeta : 11
                                    elide: Text.ElideMiddle
                                    textFormat: Text.PlainText
                            ...
                            Text {
                                ...
                                horizontalAlignment: Text.AlignRight
                                text: modelData.bytes >= 0 && root.service ? root.service.formatBytes(modelData.bytes) : ""
                                color: root.t ? root.t.fainter : "grey"
                                font.family: root.t ? root.t.family : "monospace"
                                font.pixelSize: root.t ? root.t.fMeta : 11
                                textFormat: Text.PlainText
                ...
                Repeater {
                    ...
                    delegate: Column {
                        ...
                        Item {
                            ...
                            Rectangle {
                                anchors.fill: parent
                                color: groupMouse.containsMouse ? (root.t ? root.t.hair(0.03) : "transparent") : "transparent"
                        ...
                        Column {
                            ...
                            Repeater {
                                ...
                                delegate: Text {
                                    ...
                                    text: Model.ruleProgram(modelData) + "  ·  " + Model.ruleScopeWords(modelData.name, modelData.path || "") + "\n" + modelData.sourceName + "  ·  " + modelData.name
                                    color: root.t ? root.t.faint : "grey"
                                    font.family: root.t ? root.t.family : "monospace"
                                    font.pixelSize: root.t ? root.t.fMeta : 11
