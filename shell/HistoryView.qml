...
Item {
  ...
  readonly property var shown: Model.filterHistory(
    root.filter === "covered" ? root.coveredIncidents : root.incidents, root.filter)
  readonly property var days: Model.historyDays(root.shown,
  ...
  Text {
    ...
    text: {
      if (root.service && !root.service.logReadable) return "Moat cannot read its own record of what happened."
      if (root.filter === "needsYou") return "Nothing has needed a decision."
      if (root.filter === "covered") return "No rule has silenced anything yet."
          ...
          delegate: Column {
            ...
            Item {
              ...
              MouseArea {
                ...
                onClicked: root.openReceipt = root.openReceipt === String(modelData.key)
            ...
            Column {
              ...
              readonly property var block: root.service
                ? root.service.receiptBlock(modelData) : ({ headline: "", lines: [] })
