...
Item {
    ...
    MoatScroll {
        ...
        Column {
            ...
            Text {
                width: parent.width
                text: root.needsPackage ? "The omarchy-moat package provides Tetragon, the policies, and the moatd daemon. Until it is installed and its services are running there is nothing to monitor." : "The control socket (/run/moat/control.sock) and the alert log (/var/lib/moat/alerts.jsonl) are both group-readable by `moat` only. Without that membership this panel cannot read alerts or act on them."
