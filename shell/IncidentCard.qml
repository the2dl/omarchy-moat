...
Rectangle {
  ...
  function chooseScope(n) { if (root.scopeOpen) scopeChoice.chooseIndex(n) }
  function toggleEvidence() { root.evidenceOpen = !root.evidenceOpen }
  ...
  readonly property var rotateItems: root.service && root.head
    ? root.service.rotateItems(root.head) : []
  /// The recorded fields, label/value flattened. Empty unless Advanced is on,
  /// so the default card allocates nothing for it.
  readonly property var rawFacts: root.raw && root.service && root.head
    ? root.service.rawFacts(root.head) : []
  ...
  Column {
    ...
    Row {
      ...
      Text {
        text: root.chain ? Copy.chainRuleNote(root.chain)
                         : (root.incident ? root.incident.rule : "")
    ...
    Text {
      ...
      text: {
        ...
        if (root.head && root.head.rarity === "first_seen") bits.push("first time on this machine")
        if (root.head && root.head.action_taken === "none") bits.push("nothing has been stopped")
        return bits.join("  ·  ")
      }
    ...
    Item {
      ...
      Rectangle {
        ...
        color: root.t ? (root.chain ? root.t.alarmTint(0.5) : root.t.accentTint(0.4))
                      : "transparent"
      }
    ...
    Grid {
      ...
      Repeater {
        ...
        delegate: Text {
          ...
          width: index % 2 === 0 ? (root.t ? root.t.s(120) : 90)
                                 : card.width - (root.t ? root.t.s(136) : 98)
          text: modelData
    ...
    // -------------------------------------------------- what to change now
    //
    // CONTRACT 4's `rotate[]`. When a credential was read, the only thing that
    // actually helps is replacing it, and no decision the user makes in this
    // panel does that -- allowing it does not un-read the key, and stopping the
    // process does not either. It sits above the actions for that reason.
    Column {
      width: parent.width
      ...
      Text {
        text: "CHANGE THESE, WHATEVER YOU DECIDE HERE"
        color: root.t ? root.t.accent : "orange"
        font.family: root.t ? root.t.family : "monospace"
        font.pixelSize: root.t ? root.t.fMeta : 11
    ...
    Flow {
      ...
      Text {
        visible: !!root.head && root.head.actions
                 && root.head.actions.indexOf("ignore") === -1
                 && root.head.actions.indexOf("kill") === -1
        width: parent.width
      ...
      Button {
        ...
        foreground: root.t ? root.t.dimmer : "grey"
        fontSize: root.t ? root.t.fSecondary : 12
        onClicked: { root.scopeOpen = false; root.bundleOpen = true }
      }
    ...
    EvidenceBlock {
      ...
      service: root.service
      alert: root.head
      onCopyBundleRequested: function (id) { root.copyBundle(id) }
    }
    ...
    Text {
      ...
      text: {
        ...
        if (root.service.copyMessage) return String(root.service.copyMessage)
        if (root.service.analyzeMessage) return String(root.service.analyzeMessage)
        return ""
      }
      color: root.t ? root.t.faint : "grey"
      font.family: root.t ? root.t.family : "monospace"
  ...
  readonly property bool pendingVerdict: !root.verdict && !!root.head
    && !!root.service && root.service.autoTriageOn === true
  /// A verdict is shown, but a newer occurrence has not been looked at yet.
  readonly property bool stillReading: !!root.verdict && !!root.incident
    && root.incident.awaitingVerdict === true
    && !!root.service && root.service.autoTriageOn === true

  function factTarget() {
    if (!root.head) return ""
    if (root.head.file && root.head.file.path) return String(root.head.file.path)
