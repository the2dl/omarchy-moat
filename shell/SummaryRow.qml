...
Item {
  ...
  readonly property color stateColor: {
    ...
    case "explained": return root.t.muted
  ...
  Rectangle {
    anchors.fill: parent
    color: mouse.containsMouse && root.showChevron
  ...
  Row {
    ...
    Text {
      id: detailText
      anchors.verticalCenter: parent.verticalCenter
      width: parent.width - labelText.width - stateText.width - chevron.width
             - (root.t ? root.t.s(16) * 3 : 24)
      text: root.detail
    ...
    Text {
      ...
      text: {
        ...
        if (root.severity) word = root.severity + " · " + word
        if (root.count > 1) return root.count + "x  " + word
        return word
      }
