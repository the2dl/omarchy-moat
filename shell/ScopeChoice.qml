...
Rectangle {
  ...
  readonly property var scopes: root.head
    ? Model.silenceScopes(root.head, root.incident.count) : []
  ...
  readonly property var chosenScope: {
    for (var i = 0; i < root.scopes.length; i++) {
      if (root.scopes[i].scope === root.chosen) return root.scopes[i]
    }
    return root.scopes.length > 0 ? root.scopes[0] : null
  }

  onScopesChanged: {
    if (root.scopes.length === 0) { root.chosen = ""; return }
    for (var i = 0; i < root.scopes.length; i++) {
      if (root.scopes[i].scope === root.chosen) return
    }
  ...
  function chooseIndex(n) {
    var i = Number(n) - 1
    if (i >= 0 && i < root.scopes.length) root.chosen = root.scopes[i].scope
  }

  function confirm() {
    if (!root.incident || !root.chosenScope) return
  ...
  Column {
    ...
    Text {
      width: parent.width
      text: root.incident && root.incident.count > 1
        ? "This keeps firing. Stop asking about it?"
        : "Say this was you. How wide should that be?"
