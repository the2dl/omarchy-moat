...
Item {
  ...
  readonly property int controlWidth: Math.max(root.minControlWidth,
    controlHolder.children.length > 0 ? controlHolder.children[0].implicitWidth : 0)

  implicitHeight: Math.max(text.implicitHeight, controlHolder.childrenRect.height)
                  + (root.t ? root.t.s(24) * 2 : 24)
  ...
  Column {
    ...
    Text {
      ...
      visible: text !== ""
      text: root.help
      color: root.helpLoud ? (root.t ? root.t.alarm : "red")
