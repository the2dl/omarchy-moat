...
Rectangle {
  ...
  readonly property var learning: Model.learningCard(root.status,
    root.service ? root.service.nowMs : Date.now())
  readonly property var done: Model.learningDoneCard(root.status,
    root.service ? root.service.nowMs : Date.now())
  ...
  Column {
    ...
    Item {
      ...
      Text {
        ...
        text: root.learning
          ? root.learning.learned + (root.learning.learned === 1 ? " thing learned"
                                                                 : " things learned")
          : ""
        color: root.t ? root.t.fainter : "grey"
        font.family: root.t ? root.t.family : "monospace"
    ...
    Column {
      ...
      Repeater {
        model: root.done ? [
          { label: "Written down as normal", value: String(root.done.learned) },
          { label: "Detections gone quiet", value: String(root.done.demoted) },
          { label: "Waiting for you to say yes", value: String(root.done.proposals) }
        ] : []
