...
Item {
  ...
  readonly property int cursorIndex: {
    if (root.queue.length === 0) return -1
    for (var i = 0; i < root.queue.length; i++) {
      if (root.queue[i].key === root.cursorKey) return i
    }
    return 0
  }
  ...
  function step(delta) {
    ...
    if (i >= root.queue.length) i = 0
    root.cursorKey = root.queue[i].key
  }
  function showIncident(key) { root.cursorKey = String(key) }
  readonly property var t: root.tokens

  readonly property color toneColor: {
    if (!root.t) return "transparent"
    switch (root.verdict.tone) {
    case "alarm": return root.t.alarm
    case "accent": return root.t.accent
    default: return root.t.calm
    }
  }
  ...
  readonly property var categories: {
    ...
    for (var i = 0; i < root.incidents.length; i++) {
      ...
      if (inc.state === "needsYou") continue
      var fam = String(inc.family || "other")
      if (!byFamily[fam]) byFamily[fam] = { family: fam, count: 0, state: inc.state, programs: {} }
      ...
      if (inc.state === "explained") byFamily[fam].state = "explained"
    }
    var labels = {
      pkg: "Package installs", persist: "Writes to your config",
      cred: "Reads of your keys", exec: "Programs that ran",
      net: "Network connections", priv: "Requests for more access",
      rootkit: "Kernel-level changes", shell: "Shells that opened",
      x: "Moat itself", other: "Other"
    }
    ...
    Column {
      ...
      Repeater {
        ...
        delegate: BlockedCard {
          ...
          onAllowRequested: function (id, scope) { root.requestWasMe(id, scope) }
          onMonitorRequested: function (rule) { root.requestMonitor(rule) }
          onCloseIt: function (id) { root.requestClose(id) }
        }
      }
      ...
      IncidentCard {
        ...
        onWasMe: function (id, scope) { root.requestWasMe(id, scope) }
        onCloseIt: function (id) { root.requestClose(id) }
        onStopIt: function (id) { root.requestStop(id) }
        onAskMoat: function (id) { root.requestAsk(id) }
        onCopyBundle: function (id) { root.requestCopyBundle(id) }
        onSaveBundle: function (id) { root.requestSaveBundle(id) }
        onCopyBundleText: function (id) { root.requestCopyBundleText(id) }
        onAllowChain: function (id) { root.requestAllowChain(id) }
      }
      ...
      Column {
        ...
        Repeater {
          ...
          model: root.queue.filter(function (q) { return q.key !== root.cursorKey })
  ...
  function footerFacts() {
    ...
    var bits = []
    bits.push(root.status.mode === "enforce" ? "Blocking, not just watching"
  ...
  function summaryLine() {
    ...
    for (var j = 0; j < root.incidents.length; j++) {
      if (root.incidents[j].state === "explained") explained++
    }
    if (seen > 0) {
      bits.push("Moat looked at " + seen + (seen === 1 ? " thing" : " things") + " today.")
    }
    if (explained > 0) {
      bits.push(explained === 1
        ? "One was worth a second look; it read it and closed it."
        : explained + " were worth a second look; it read them and closed them.")
    }
    ...
    var armed = root.status && Array.isArray(root.status.enforceable)
      ? root.status.enforceable.filter(function (r) { return r.armed }).length : 0
    if (stopped > 0) {
      bits.push(stopped === 1
        ? "One thing was stopped."
        : stopped + " things were stopped.")
    } else if (armed > 0) {
      bits.push("Nothing has been stopped. " + armed
                + (armed === 1 ? " rule is armed" : " rules are armed")
                + "; everything else is watched only.")
