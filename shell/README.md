...
| `QuarantineView.qml` | The Quarantine tab: every file moat is holding, with where it came from, which rule took it, when, its size, and the path it is held at so it can be inspected with any tool. Restore puts it back; the daemon refuses if the original path is occupied again or the held bytes no longer hash to what was recorded, so the button cannot overwrite anything or hand back a file that changed while held. A held file that has vanished from disk is called out in urgent, because that means something removed it out from under moat. |
