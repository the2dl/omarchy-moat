...
| moatd | `cd moatd && cargo test --release` | **PASS** — 131 unit tests (was 128; three added) + 5 integration tests |

Counts before this review: shell 39, sandbox 52, moatd 128+5. After: shell
58, sandbox 53, moatd 131+5.
...
`moatd/dev-run.sh` renders `testdata/templates` (5 policies) and replays
...
{"event_set":["PROCESS_KPROBE","PROCESS_LSM","PROCESS_TRACEPOINT","PROCESS_UPROBE"],
 "policy_names":["moat-ai-cli-in-pkg-subtree", ... 32 names ... ]}

$ moatd run --log $D/tetragon.log --state-dir $D/state --socket $SOCK \
...
$ moatctl list --json | jq -r '.alerts[] | "\(.id) \(.family) \(.action_taken)"'
...
$ moatctl explain 01M1M5XHBZTEYJ03KHCF0R5P32
Private SSH key read by an unexpected program
=============================================
moat-cred-ssh-private-key-read (high)   2026-09-03T17:43:56.543Z   alert 01M1M5XHBZ...
...
EVIDENCE
  ...
  - not in allowlist: none of the 0 rule(s) in .../allowlist.d matches
    exe=/usr/bin/node file=/home/moattest/.ssh/id_ed25519

IF THIS IS EXPECTED
  ... (recommended scope: exe)
  * moatctl ignore 01M1M5XHBZ... --scope exe
      [[rule]]
...
WHAT TO DO
  1. If you did not expect this: stop it now with `moatctl kill <id>` ...
...
$ moatctl ignore 01M1M5XHBZ... --scope exe --comment "dev demo"
...
$ moatctl allowlist
  1  moat-cred-ssh-private-key-read
...
$ moatctl unignore 1
removed from .../allowlist.d/user.toml: [[rule]] name = "moat-cred-..."

$ moatctl set mode monitor          # tetra does not exist
mode is now monitor (0 of 32 policies updated via .../no-such-tetra)
could not set the mode on 32 policies:
  moat-ai-cli-in-pkg-subtree: No such file or directory (os error 2)
  ... 31 more ...
$ echo $?
0                                        <- see mismatch M5

$ moatctl set sandbox on --json
{"sandbox": true, "flag": ".../etc/sandbox.enabled",
 "note": "shims apply to new login sessions; log out and back in", "ok": true}
$ moatctl status --json | jq .sandbox
true
```
...
```
{"v":1,"id":"01M1M5XHBZTEYJ...","severity":"high","rule":"moat-cred-ssh-private-key-read",...}
{"v":1,"id":"01M1M5XHBZTEYJ...","update":{"action_taken":"killed"}}
{"v":1,"id":"01M1M5XHBZ9F1G...","severity":"high","rule":"moat-persist-shell-rc-write",...}
{"v":1,"id":"01M1M5XHBZKWBC...","severity":"high","rule":"moat-pkg-subtree-downloader",...}
{"v":1,"id":"01M1M5XHBZVPVK...","severity":"high","rule":"moat-cred-ssh-private-key-read",...}
...
CONTRACT §2, and the socket group became `moat`.
...
runs `MoatModel.js` over the fixtures above — real daemon output, not
...
`policies/*.yaml` annotate 20 distinct `moat.omarchy/rotate` values.
...
* `moatd/src/explain.rs` — `rotate_advice()` covers all 20, with the old
  ...
  table does not know.
* `shell/MoatModel.js` — `ROTATE_GUIDANCE` covers all 20, aliases kept.
...
|---|---|---|
| `rule.path` (the rule's `file =` glob) | `rule.file` | the `file =` matcher never appeared in the rule's detail line; worse, `rule.file` in the response is the *source fragment* the rule came from, so an `exe+file` rule would have shown `/etc/moat/allowlist.d/user.toml` as its match target |
| `rule.toml` (the rendered block) | `rule.line` | the exact `[[rule]]` block was never available to the view |
| `user_file` (the file `unignore` edits) | `file` | fell back to a hardcoded `/etc/moat/allowlist.d/user.toml`, right in production, wrong under any override |

CONTRACT §5 does not fix these names, so per the review brief the cheaper side
changed. `shell/MoatModel.js` now accepts both spellings, and
...
`moatd/src/explain.rs` now branches on the hook: `"A package install
...
Exit code 0. The failure is enumerated, not swallowed, and `moatctl`
...
a failure and names it — *"moatd recorded mode enforce but could not apply
it to any of 32 policies … Tetragon is still in the previous mode."*

### M6 — `MOAT_SANDBOX=0` did not bypass the PKGBUILD scan (fixed, sandbox)

CONTRACT §8 ends the shim paragraph with "`MOAT_SANDBOX=0` bypasses."
`moat-scan-pkgbuild`'s own footer prints:

```
build anyway          MOAT_SANDBOX=0 makepkg -si   # skips the sandbox and this scan
```

…and the shim's refusal message says "set `MOAT_SANDBOX=0` to bypass
...
`MOAT_SANDBOX=0` and a fixture PKGBUILD scoring high, the shim still
...
`MOAT_SANDBOX=0`. `MOAT_SANDBOX_ACTIVE=1` deliberately still scans: a
nested `makepkg` is usually a different directory with a different PKGBUILD the
outer call never saw. New sandbox test *"MOAT_SANDBOX=0 skips the PKGBUILD
...
`"socket_group":"moat"` in the example response.
...
`[moat] PKGBUILD scan: medium findings above. Continuing.` then build; exit 2
...
`proceed` text names `~/.config/moat/scanner-allow.conf` and
`/etc/moat/scanner-allow.conf`, both exactly as CONTRACT §2 spells them,
...
`shell/`, `sandbox/`, `scanner/`, `moatd/src/config.rs` defaults,
`moatd/etc/`, `moatd/systemd/`, `policies/tetragon.conf.d/` and
`policies/tetragon.service` agrees with §2. Checked pairwise: the daemon's
`tetragon_log` default is `/var/log/moat/tetragon.log` = conf.d's
`export-filename`; `policies_dir` is `/run/moat/policies` = conf.d's
`tracing-policy-dir`; `templates_dir` is `/usr/lib/moat/policies`;
...
The plugin's `/usr/bin/moatctl`, `/var/lib/moat/alerts.jsonl` and
`/etc/moat/allowlist.d/user.toml` all match. Two paths appear that §2 does
not list — see §6.

**(f) `MOAT_SANDBOX_ACTIVE=1`.** Set by `moat-sandbox` via
...
daemon will never contain `MOAT_SANDBOX_ACTIVE`. Widening the filter to
`LD_PRELOAD,MOAT_SANDBOX_ACTIVE` would make it visible, but it should not be
done: the variable is trivially forgeable by the very code the pkg policies
watch (`env MOAT_SANDBOX_ACTIVE=1 curl …` inside a postinstall script would
...
`ExecStartPre=/usr/bin/moatd render-policies` with no flags, and every
default the subcommand falls back to matches §2: templates
`/usr/lib/moat/policies`, out `/run/moat/policies`, export-allowlist
`/etc/tetragon/tetragon.conf.d/export-allowlist`, passwd `/etc/passwd`. The
unit's `ProtectSystem=strict` +
`ReadWritePaths=/run/moat … /etc/tetragon/tetragon.conf.d` covers both
...
1. **CONTRACT §5's `status` example is stale.** It shows `"group_ok":true`; the
   daemon ships `"socket_group":"moat"`. The daemon's reading is the better
...
2. **`set mode` still returns `ok:true` when it applied to zero policies.**
   Worked around in the plugin (M5), which is the cheap side, but `moatctl
...
3. **`set mode` returns 32 identical error objects** when `tetra` is absent.
   `moatctl` prints all 32 lines. Collapsing identical errors into one entry
...
4. **The makepkg shim's "continue anyway" is not recordable.** The scanner tells
   ...
   `~/.config/moat/scanner-allow.conf`, but when they answer *y* at the
   ...
   to what `moatctl ignore` does for alerts.)

5. **Two installed paths are not in CONTRACT §2**, so the packaging agent may
   miss them: `sandbox/fish/conf.d/moat-shims.fish` (the fish equivalent of
   `/etc/profile.d/moat-shims.sh`, which needs to land in
   `/usr/share/fish/vendor_conf.d/moat-shims.fish`) and
   `/usr/share/doc/omarchy-moat/README.md`, referenced from
...
6. **`quarantine` was not exercised end to end.** The scenario used a fake home
   ...
   000` and the `meta.json` are covered by `moatd`'s own unit tests but not
   against a live daemon.

7. **The `feeds` seam is untested against the network.** `moat-feeds`
   without an abuse.ch `Auth-Key` is a clean no-op (covered by
   `moat_feeds_without_a_key_is_a_no_op`), and `status.feeds` reads
...
| File | Change |
|---|---|
| `moatd/src/explain.rs` | M1 rotate table (20 kinds + aliases), M3 pkg `what`/summary; 3 new tests, two of which read `policies/` |
| `shell/MoatModel.js` | M1 rotate guidance, M2 allowlist field aliases + `removable`/`sourceFile`, M4 `ts` in `UPDATABLE`, M7 `socket_group` + `setupSteps(…, group)` |
...
| `sandbox/shims/makepkg` | M6 `MOAT_SANDBOX=0` skips the scan |
| `sandbox/tests/run.sh` | M6 regression test |
| `sandbox/README.md` | expanded the `MOAT_SANDBOX_ACTIVE` / `MOAT_SANDBOX=0` env table entries |
| `tests/run-all.sh` | new — every suite in order, non-zero on any failure |
| `docs/INTEGRATION.md` | this file |

`pkg/` was not touched. `policies/`, `scanner/` and `moatd/src/` outside
