...
|------------------------------|------------------------------------------------------------------------------------------------|----------|
| `moat-x-ai-cli-headless` | an AI CLI (`claude`,`codex`,`gemini`,`opencode`,`q`,`amp`) has no terminal/tmux/ssh/editor **and** no interactive shell outside a package subtree in its ancestry | high |
...
`MOAT_CONFIG` / `MOAT_FEEDS_CONFIG` for the two config files.
...
etc/            moat.toml, feeds.toml, allowlist.d/default.toml
```
