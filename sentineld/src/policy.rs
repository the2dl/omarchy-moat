    ...
    pub fn fallback(name: &str) -> PolicyMeta {
        PolicyMeta {
            ...
            why: "This policy is loaded in Tetragon but carries no \
                  sentinel.omarchy/why annotation, so there is no explanation to show."
                .into(),
...
/// `sentinel-cred-ssh-private-key-read` -> `cred`. `sentinel-x-mass-read` -> `x`.
pub fn family_of(name: &str) -> String {
    let mut it = name.split('-');
    match (it.next(), it.next()) {
        (Some("sentinel"), Some(f)) => f.to_string(),
        _ => "other".to_string(),
    }
}
...
metadata:
  ...
  annotations:
    ...
    sentinel.omarchy/rotate: "ssh-key, github-token"
    ...
    sentinel.omarchy/why: >-
      ...
      stealers read.
    sentinel.omarchy/expected: "Backup tools."
...
"#,
        ...
        let m = set.get("sentinel-cred-ssh-private-key-read").unwrap();
    ...
    fn fallback_is_still_usable() {
        let m = PolicyMeta::fallback("sentinel-net-reverse-shell");
