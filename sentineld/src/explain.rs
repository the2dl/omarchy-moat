...
/// and userland `sentinel-x-*` rules both fill this in.
#[derive(Debug, Clone)]
pub struct Finding {
    pub rule: String,
    pub meta: PolicyMeta,
    /// `file_post_open`, `tcp_connect`, or `userland` for a `sentinel-x-*` rule.
    pub hook: String,
...
fn if_expected(f: &Finding, id: &str, allowlist_file: &str) -> IfExpected {
    ...
    let mut push = |scope: &str, spec: RuleSpec| {
        options.push(ExplainOption {
            ...
            cmd: format!("sentinelctl ignore {} --scope {}", id, scope),
...
/// Rotation guidance, keyed by the policy's `sentinel.omarchy/rotate` values.
///
...
fn next_steps(f: &Finding) -> Vec<String> {
    ...
    if f.meta.actions.iter().any(|a| a == "kill") {
        next.push(
            "If you did not expect this: stop it now with `sentinelctl kill <id>` (kills the recorded process tree), and `sentinelctl quarantine <id>` to move the file out of reach."
                .to_string(),
        );
    } else {
        next.push(
    ...
    next.push(
        "If this was you, run one of the ignore commands above; otherwise `sentinelctl ack <id>` once you have finished checking."
            .to_string(),
    );
    next
}
...
mod tests {
    ...
    fn finding() -> Finding {
        let mut meta = PolicyMeta::fallback("sentinel-cred-ssh-private-key-read");
        ...
        let mut f = Finding::new("sentinel-cred-ssh-private-key-read", meta, proc());
    ...
    fn alert() -> Alert {
        build_alert(
            ...
            "/etc/sentinel/allowlist.d/user.toml",
            &allowlist_note(
                "/etc/sentinel/allowlist.d",
                3,
                "/usr/bin/node",
    ...
    fn every_scope_carries_a_command_and_a_toml_block() {
        ...
        for o in &a.explain.if_expected.options {
            assert_eq!(
                o.cmd,
                format!("sentinelctl ignore 01J8ZK6B4Q3M7N9P2R5S8T1V4W --scope {}", o.scope)
            );
            assert!(o.line.starts_with("[[rule]]\n"));
            assert!(o.line.contains("name = \"sentinel-cred-ssh-private-key-read\""));
    ...
    /// introduces a new `sentinel.omarchy/rotate` value, this fails until the
    ...
    fn policies_rotate_vocabulary_is_covered() {
        ...
        let Some(dir) = dir.filter(|d| d.is_dir()) else {
            // Building from a source tarball that ships only sentineld/.
            return;
        };
    ...
    /// back to "node matched the rule sentinel-…", which explains nothing.
    #[test]
