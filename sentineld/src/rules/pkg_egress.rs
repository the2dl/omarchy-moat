...
pub const ID: &str = "sentinel-x-pkg-egress";
    ...
    fn meta(&self) -> PolicyMeta {
        meta(
            ...
            "Registries and mirrors whose address ranges are not in the list yet, telemetry \
             ...
             the CIDR to `net.registry_cidrs` in sentinel.toml once you recognise it.",
            &[],
            &["kill", "ignore"],
            "rule",
        )
...
mod tests {
    ...
    fn sock_event(daddr: &str, dport: u16) -> HookEvent {
        HookEvent {
            ...
            policy_name: Some("sentinel-net-egress".into()),
            ..Default::default()
        }
    }
