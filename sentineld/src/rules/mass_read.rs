...
pub const ID: &str = "sentinel-x-mass-read";
...
impl UserRule for MassRead {
    ...
    fn on_hook(&mut self, h: &HookHit, exec_id: &str, ctx: &RuleCtx) -> Vec<Finding> {
        ...
        if !h.policy_name().starts_with("sentinel-cred-") {
            return Vec::new();
        }
...
mod tests {
    ...
    fn the_same_file_read_repeatedly_is_not_a_sweep() {
        ...
        let mut rule = MassRead::default();
        let e = ev("/home/dan/.ssh/id_rsa", 4, "sentinel-cred-ssh-private-key-read");
        for _ in 0..20 {
    ...
    fn writes_and_non_dotdir_paths_are_ignored() {
        ...
        for i in 0..10 {
            // MAY_WRITE only.
            let w = ev(&format!("/home/dan/.cache/w{}", i), 2, "sentinel-cred-dotfile-read");
            assert!(fire(&mut rule, &t, &c, &w, 100).is_empty());
            // Not a dotdir.
            let p = ev(&format!("/home/dan/proj/f{}", i), 4, "sentinel-cred-dotfile-read");
    ...
    fn only_the_cred_family_feeds_this_rule() {
        ...
        let mut rule = MassRead::default();
        for i in 0..10 {
            let e = ev(&format!("/home/dan/.config/x{}", i), 4, "sentinel-priv-something");
            assert!(fire(&mut rule, &t, &c, &e, 100).is_empty());
        }
    }
    ...
    fn the_finding_names_the_files_it_saw() {
        ...
        for i in 0..4 {
            let e = ev(
                ...
                4,
                "sentinel-cred-ssh-private-key-read",
            );
