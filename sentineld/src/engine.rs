...
impl Daemon {
    ...
    /// A `sentinel-*` policy event becomes a finding; anything else is
...
mod tests {
    ...
    fn dev_daemon(dir: &Path) -> (Daemon, Config) {
        ...
        let d = Daemon::new(cfg.clone(), &dir.join("sentinel.toml")).unwrap();
        (d, cfg)
    }
    ...
    fn the_sample_log_produces_explainable_alerts() {
        ...
        assert!(rules.contains(&"sentinel-cred-ssh-private-key-read"));
        assert!(rules.contains(&"sentinel-net-reverse-shell"));
        assert!(rules.contains(&"sentinel-x-ai-cli-headless"));
    ...
    fn ancestry_reaches_the_package_manager() {
        ...
        let a = d
            ...
            .load()
            .into_iter()
            .find(|a| a.rule == "sentinel-cred-ssh-private-key-read")
            .unwrap();
    ...
    fn a_kill_is_only_recorded_after_the_exit_signal() {
        ...
        let a = d
            ...
            .load()
            .into_iter()
            .find(|a| a.rule == "sentinel-net-reverse-shell")
            .unwrap();
    ...
    fn the_allowlist_suppresses_an_alert() {
        ...
        assert!(!d
            ...
            .load()
            .iter()
            .any(|a| a.rule == "sentinel-cred-ssh-private-key-read"));
    }
    ...
    fn repeats_inside_the_window_fold_into_a_count() {
        ...
        let line = std::fs::read_to_string(&d.cfg.paths.tetragon_log)
            .unwrap()
            .lines()
            .find(|l| l.contains("sentinel-cred-ssh-private-key-read"))
            .unwrap()
            .to_string();
        for _ in 0..5 {
            d.handle_line(&line);
        ...
        let alerts: Vec<Alert> = d
            ...
            .load()
            .into_iter()
            .filter(|a| a.rule == "sentinel-cred-ssh-private-key-read")
            .collect();
    ...
    #[test]
    fn a_non_sentinel_policy_is_enrichment_only() {
        let dir = tempfile::tempdir().unwrap();
        let (mut d, _) = dev_daemon(dir.path());
    ...
    fn status_has_every_contract_field() {
        ...
        assert_eq!(s["socket_group"], "sentinel");
    ...
    fn state_json_is_written_and_mode_survives_a_restart() {
        ...
        let d2 = Daemon::new(cfg.clone(), &dir.path().join("sentinel.toml")).unwrap();
