//! `sentinelctl` — a thin CLI over the control socket.
//!
//! It gains nothing from root: the socket is 0660 root:sentinel and every
...
use sentineld::alert::Alert;
use sentineld::config::Config;
use sentineld::control::request;

#[derive(Parser)]
#[command(
    name = "sentinelctl",
    version,
    about = "Inspect and act on Sentinel alerts",
    after_help = "Every action names an alert id, never a pid or a path.\n\
                  If you get a permission error: sudo usermod -aG sentinel $USER, then log out and back in."
)]
struct Cli {
    /// Control socket (default /run/sentinel/control.sock).
    #[arg(long, global = true)]
    socket: Option<PathBuf>,
    /// sentinel.toml, used only to find the socket.
    #[arg(long, global = true)]
...
enum Cmd {
    ...
    Kill { id: String },
    /// Move the alert's file into /var/lib/sentinel/quarantine.
...
fn print_human(cmd: &Cmd, r: &Value) {
    ...
    match cmd {
        ...
        Cmd::Explain { .. } => match serde_json::from_value::<Alert>(r["alert"].clone()) {
            ...
            Err(e) => eprintln!("sentinelctl: unreadable alert: {}", e),
        },
        ...
        Cmd::Ignore { .. } => {
            ...
            println!("Undo with: sentinelctl allowlist, then sentinelctl unignore <index>");
        }
...
fn print_status(r: &Value) {
    let u = &r["unacked"];
    println!("sentineld  {}   mode {}", r["version"].as_str().unwrap_or("?"), r["mode"].as_str().unwrap_or("?"));
    ...
    println!(
        ...
        r["socket_group"].as_str().unwrap_or("sentinel"),
        r["uptime_secs"],
        r["events_seen"],
        r["alerts"]
    );
...
fn print_list(r: &Value) {
    ...
    println!("\n{} alert(s). `sentinelctl explain <id>` for the full story.", alerts.len());
}
...
fn print_allowlist(r: &Value) {
    ...
    println!(
        "\nRemove one with `sentinelctl unignore <index>` (indexed rules live in {}).",
