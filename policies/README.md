...
| `moat-shell-reverse-shell-connect` | medium | `tcp_connect` (kprobe) | **Sigkill when explicitly armed; monitor recommended** | a script using `bash /dev/tcp` as a port check against a public host; `nc`/`socat` used deliberately. Private, loopback and link-local destinations never fire here — the private half is `moat-shell-lan-connect` | `matchBinaries Postfix` list; add CIDRs to `NotDAddr` |
| `moat-shell-lan-connect` | medium | `tcp_connect` (kprobe) | Post 60s/process | the same shell/netcat list, but to 10/8, 172.16/12, 192.168/16, shared 100.64/10 or `fc00::/7`. **Hand-run port checks are the whole FP surface**: `bash -c ": < /dev/tcp/192.168.1.1/80"` wait-loops, `nc -z` scans against a NAS, printer, router or container host, wait-for-it.sh in a Makefile. Never kills; loopback and link-local excluded | `matchBinaries Postfix` list; drop CIDRs from `DAddr` |
...
| `moat-net-tmpfs-binary-egress` | medium | `socket_connect` (LSM) | **Override / EPERM when explicitly armed** | installers, build tools, or coding agents staging tools such as Helm in `/tmp`; use monitor mode for ordinary developer workflows | `matchBinaries Prefix` values; per-rule monitor/enforce switch |
