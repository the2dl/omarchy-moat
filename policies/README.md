...
| `moat-net-tmpfs-binary-egress` | critical | `socket_connect` (LSM) | **Override / EPERM when explicitly armed** | installers, build tools, or coding agents staging tools such as Helm in `/tmp`; use monitor mode for ordinary developer workflows | `matchBinaries Prefix` values; per-rule monitor/enforce switch |
